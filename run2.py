import socket,struct,time,os,sys

def slowprint(s):
    for c in s + '\n':
        sys.stdout.write(c)
        sys.stdout.flush()
        time.sleep(0.1 / 2)
slowprint (' ASSALAMUALAIKUM WARAHMATULLAHI WABARAKATUH ')
slowprint (' SYALOM, OM SWASTIYASTU, SALAM SEJAHTERA ')

print
slowprint(' Selamat Datang Sob Di TOOLS INSTALLER GW ')
slowprint(' Gunakan Tools Dengan Bijak, Dan Mohon Untuk Kalian ')
slowprint(' Yang Tahu Username & Password Tools Ini, Mohon ')
slowprint(' Untuk Tidak Menyebarluaskan, Biarkan Mereka Yang ')
slowprint(' Usaha Mencari Username & Passwordnya Sendiri ')
slowprint(' Jangan Jadi Orang Yang Pemalas ')
slowprint(' Gampang Njir!! Tinggal ketik 2 Kalian Tonton Vidio Nya Sampai Habis ')
print
slowprint(' Hargai Author')
slowprint(' Jelajahi Setiap Toolsnya Dan selamat bersenang-senang ')
print
slowprint(' JANGAN LUPA SUBSCRIBE CHANNEL GW ')
slowprint(' FOLLOW IG JUGA GW mr.tamfanx ')
slowprint(' THANKS FOR All My Friends & Indonesian People & You ')
slowprint(' Nb : Klo lu Mau Recode ingat!! Recode gak akan jadiin lu Programer ')
slowprint(' Stay Humble Work Hard Pray Hard Nothing Imposible In world ')


os.system('sh loginv2.sh')

exit()
